        ...
        public void draw(java.awt.Graphics g) {
            ...
            for(int i = 0; i < game.getListener().getGoodOrBadChar().length; i++){
                for(int j = 0; j < game.getListener().getGoodOrBadChar()[i].length; j++){
                    ...
                    g.drawString(game.getListener().getText().getWords().get(i).charAt(j) + "", x, y);
                    if(game.getListener().getText().getWords().get(i).charAt(j) == 'i' || game.getListener().getText().getWords().get(i).charAt(j) == 'l') x += 14;
                    else if(game.getListener().getText().getWords().get(i).charAt(j) == 'm') x += 27;
                    else x += 22;
                }
                x += 5;
                if(i != 0 && i % 4 == 0){
